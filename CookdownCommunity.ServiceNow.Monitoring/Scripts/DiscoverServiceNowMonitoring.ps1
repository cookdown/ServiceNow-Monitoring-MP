...
Function DebugDiscovery
{
	...
	if($debugDiscovery)
	{
        ...
        {
            $api.LogScriptEvent("SNOW Discover", 1020, 4, "Discovery DEBUG:`n$messageString`nWith a value of: $objectValue")
        }
        else
        {
            $api.LogScriptEvent("SNOW Discover", 1020, 4, "Discovery DEBUG:`n$messageString")
        }
...
try
{
    foreach($instance in $instanceUrls.Trim().Split(","))
    {
        ...
        #Add watcher
		DebugDiscovery "Creating objecet for Watcher Node" $computerName
